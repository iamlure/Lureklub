...
static const int64_t nTargetTimespan = 120 * 60;  // Lure - every 2 hour
unsigned int nTargetSpacing = 1 * 60; // Lure - 1 minute
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 12 * 60 * 60; // Lure - 12 hours
unsigned int nStakeMaxAge = -1; // Lure - unlimited
unsigned int nModifierInterval = 10 * 60; // Lure - time to elapse before new modifier is computed
...
const string strMessageMagic = "Lure Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Lure", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "16 MAR 2015 - New U.S. stealth bomber shrouded in mystery-cnn.com";
        CTransaction txNew;
        txNew.nTime = 1415210670;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce   = !fTestNet ? 8870230 : 294567;

        if (false && (block.GetPoWHash() != hashGenesisBlock)) {
        ...
        assert(block.hashMerkleRoot == uint256("0xa23d533f90923de2ef60f834fa52a3cb49097613e5b7ab891c952ad74c56c7e5"));


        assert(block.GetPoWHash() == (!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet));
...
unsigned char pchMessageStart[4] = { 0xe5, 0xc5, 0xd5, 0xe7 };
